...
{
    public class MD5CrackerArgumentParser
    {
        private string[] args;
        private int position;

        public MD5CrackerArgumentParser(string[] args)
        {
            if (args.Length == 0)
                displayHelp();
            this.args = args;
        ...
        public MD5CrackerConfig Parse()
        {
            MD5CrackerConfig config = new MD5CrackerConfig();
            for (position = 0; position < args.Length; position++)
            {
                switch (args[position++].ToLower())
                {
                    ...
                    case "-g":
                    case "--generate":
                        Console.WriteLine(new MD5CrackerHashCracker(config).Hash(expectData("[STRING]")));
                        Environment.Exit(0);
                        break;
                    case "-m":
                    case "--method":
        ...
        {
            Console.WriteLine("MD5Cracker [FLAGS]");
